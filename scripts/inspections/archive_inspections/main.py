...
class InspectionArchiver:
    ...
    async def __aenter__(self):
        ...
        csv_filename = "archive_results.csv"
    ...
    async def archive_single_inspection(
        ...
        async with self.semaphore:
            ...
            url = f"{BASE_URL}/inspections/v1/inspections/{audit_id}/archive"

            for attempt in range(MAX_RETRIES):
                try:
                    # Archive endpoint requires empty JSON body
    ...
    async def archive_all_inspections(self, audit_ids: List[str]) -> Dict:
        """
        ...
        """
        print(f"\n🚀 Starting bulk archive for {len(audit_ids)} inspections...")
        ...
        print("📊 Live results: archive_results.csv\n")
        ...
        with tqdm(total=len(audit_ids), desc="Archiving", unit="inspection") as pbar:
            for batch_num in range(total_batches):
                ...
                pbar.set_description(
                    f"Archiving (Batch {batch_num + 1}/{total_batches})"
                )

                # Create tasks ONLY for this batch
                tasks = [
                    self.archive_single_inspection(audit_id, pbar) for audit_id in batch
                ]
...
def load_completed_audit_ids(csv_path: str = "archive_results.csv") -> set:
    """
...
async def main():
    print("=" * 80)
    print("🗄️  SafetyCulture Inspection Bulk Archiver")
    print("=" * 80)
    ...
    async with InspectionArchiver() as archiver:
        results = await archiver.archive_all_inspections(audit_ids)
    ...
    print(f"\n💾 Results log: {os.path.abspath('archive_results.csv')}")
    print("=" * 80)
